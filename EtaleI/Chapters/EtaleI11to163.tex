...
\section{Motivation}
\numpar{Problem} \lecture[Weil cohomology theories: some motivation, some (counter-)examples. Flatness, flat base change, faithfully flat descent.]{2019-10-18}For a scheme $X$, we would like to have cohomology groups $H^*(X,\IZ)$ with properties similar to the ones familiar from algebraic topology. For example, if $f\colon X\morphism X$ is a continuous map of a topological space into itself, then (under some sensible conditions) the \emph{Lefschetz trace formula} says
\begin{equation*}
