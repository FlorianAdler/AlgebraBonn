...
\end{fact}
\begin{proof}
	As every étale morphism with $|x|$ in its image has a section by \cref{prop:henselian}\itememph{e}, we see that $(X,x)$ is cofinal in the category of étale neighbourhoods of $x$.
\end{proof}
\begin{cor}\label{cor:finiteStalks}
	...
	\end{equation*}
	where the product is taken over \enquote{all} geometric points $x\colon \Spec \kappa(x)\morphism X$ \enquote{lying over} $y$.
\end{cor}
\begin{rem*}\label{rem:WTFlyingOver}
	In the lecture we were talking about \enquote{lifts} of $y$ to $X$, but this is definitely not what we want. For example, $Y=\Spec k$ could be the spectrum of a separably closed field and $X=\Spec \ell$ for $\ell/k$ a non-trivial purely inseparable extension. Then the identity on $Y$ is a geometric point, but admits no lift to $X$. So instead I wrote $x$ \enquote{lying over} $y$, but this too needs some explanation: obviously, $x$ lying over $y$ should include the condition that a diagram
	\begin{equation*}
		\begin{tikzcd}
		\Spec \kappa(x)\rar["x"]\dar & X\dar["f"]\\
		\Spec \kappa(y)\rar["y"] & Y
		\end{tikzcd}
	\end{equation*}
	commutes. Still we get into set-theoretic trouble since there is a whole proper class of such $x$, and we would rather have a finite set of them! There are essentially two ways out of this:
	\begin{numerate}
		\item We could demand that $\kappa(y)$ and $\kappa(x)$ both equal some chosen separable closures $\kappa(|y|)^\sep$ and $\kappa(|x|)^\sep$ of their underlying ordinary points. We have seen in \cref{rem:setTheory} that this loses no information.
		\item We could define a preorder on the class of all $x$ as above, with $x'\leq x$ iff $x\colon\Spec \kappa(x)\morphism X$ can be factored over $x'\colon \Spec \kappa(x')\morphism X$. Note that $x'\leq x$ and $x\leq x'$ implies that the fields $\kappa(x)$ and $\kappa(x')$ are isomorphic. Then we can take the finite set of isomorphism classes of $x$ that are minimal with respect to \enquote{$\leq$} (it will become clear from the proof below that such a finite set indeed exists).
	\end{numerate}
\end{rem*}
\begin{proof}[Proof of \cref{cor:finiteStalks}]
	The question is local on $Y$, whence we may assume $Y=\Spec A$, $X=\Spec B$, such that $B$ is finite over $A$. Observe that $\Oo_{Y_\et,y}$ is strictly henselian by \cref{prop:henselization}\itememph{g}. By \cref{prop:henselian}\itememph{c} thus, the finite $\Oo_{Y_\et,y}$-algebra $S=B\otimes_A\Oo_{Y_\et,y}$ may be decomposed as
	\begin{equation*}
		S=\prod_{i=1}^nS_i\,,
	\end{equation*}
	where the $S_i$ are local and finite over $\Oo_{Y_\et,y}$. Moreover, if $\mm$ and $k$ denote the maximal ideal and the residue field of $\Oo_{Y_\et,y}$, then the $S_i/\mm S_i$ are finite over $k$, hence artinian local rings. If $\mm_i$ denotes the unique prime ideal of $S_i$, then $\kappa(\mm_i)$ is finite over the searably closed field $k$, hence $\kappa(\mm_i)$ is separably closed itself. Thus, every $x_i\colon \Spec \kappa(\mm_i)\morphism X$ is a geometric point lying over $y$. Moreover, for any $x$ as in \cref{rem:WTFlyingOver}, the morphism $B\morphism\kappa(x)$ factors over $B\otimes_Ak\cong \prod_{i=1}^nS_i/\mm S_i$, which is artinian with maximal ideals $\mm_1,\dotsc,\mm_n$. Thus, the $x_i$ are indeed \enquote{the} geometric points lying over $y$ (in the sense of either one of \itememph{1} or \itememph{2}). Now we compute
	\begin{equation*}
		(f_*\Ff)_y\cong \Global(\Spec S,\Ff)\cong \prod_{i=1}^n\Global(\Spec S_i,\Ff)\cong \prod_{i=1}^n\Ff_{x_i}\,.
	\end{equation*}
	Here the first isomorphism follows from \cref{cor:(f_*F)_y} as $X_y=\Spec S$ in this case, the second follows from $\Spec S=\coprod_{i=1}^n\Spec S_i$ and the sheaf axiom, and the third isomorphism follows from \cref{fact:StrictlyHenselianTrivialCoho}, using that the $S_i$ are finite over $\Oo_{Y_\et,y}$, hence strictly henselian too by \cref{prop:henselization}\itememph{b}.
\end{proof}
\begin{cor}
...
\begin{alphanumerate}
	\item Every open cover of $X$ splits, i.e., if $X=\bigcup_{i\in I}$ is an open cover, then $\coprod_{i\in I}U_i\morphism X$ has a section. Note that this is \emph{not} the condition from the lecture; Professor Franke explained to me afterwards that he made a mistake and will probably correct this in the next lecture.
