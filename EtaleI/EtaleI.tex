...
\lecture[Injective objects in $\cat{Ab}(X_\et)$. Étale cohomology of schemes and morphisms. Some Leray spectral sequences.]{2019-12-16}Étale cohomology of schemes and morphisms between them will be constructed using the machinery of \defemph{right-derived functors}. We assume familiarity with the construction of the functors $R^iF$ for any left-exact functor $F$ between abelian categories. Since the $R^iF$ are usually computed via injective resolutions (provided such resolutions exist), it is useful to have the following proposition.
...
\end{rem}
\begin{rem}
	If $\Oo$ is a sheaf of rings on $X_\et$, then $H^i(X_\et,-)$ are also the (underlying abelian groups of the) derived functors of $\Global(X,-)\colon \cat{Mod}_\Oo\morphism \cat{Mod}_{\Global(X,\Oo)}$. I have no idea how to prove this---as far as I can see, the argument via flabby sheaves doesn't work any more---so please enlighten me or I will ask Franke next year.
\end{rem}
\begin{rem}
...
\begin{prop}
	...
	\end{gather*}
	Moreover, if $F\colon \cat{Ab}(X_\et)\morphism\cat{Ab}(Y_\Zar)$ denotes the \enquote{forgetful pushforward} functor, then there are two spectral sequences\footnote{In the lecture, we had a single spectral sequence $E_2^{p,q}=R^pf_{\Zar,*}R^q\zeta_{X,*}\Ff\converge \zeta_{Y,*}R^{p+q}f_{\et,*}\Ff$, but I have some serious doubts about that. Again, I will ask Professor Franke for clarification next year.}
	\begin{equation*}
		E_2^{p,q}=R^pf_{\Zar,*}R^q\zeta_{X,*}\Ff\converge R^{p+q}F(\Ff)\,,\quad E_2^{p,q}=R^p\zeta_{Y,*}R^qf_{\et,*}\Ff\converge R^{p+q}F(\Ff)\,.
	\end{equation*}
\end{prop}
\begin{proof*}
	All of these are instances of the Grothendieck spectral sequence for the derivatives of a composite functor. But for this we need to check that $f_*$ and $\zeta_{X,*}$ map injective objects to acyclic ones. In fact, they both even preserve injective objects! The reason is that they both have exact left-adjoints. For $f_*$ this is just $f^*$, which is exact by the explicit description of stalks in \cref{rem:f^*}\itememph{c}.
	
	For $\zeta_{X,*}$, we would expect that one can construct a left-adjoint in a similar fashion to the construction of $f^*$. In fact, the construction becomes much easier due to the fact that every étale morphism $U\morphism X$ has open image by \cref{prop:ppfOpen}. Thus, we may define a functor $\zeta_X^\sharp\colon \cat{PAb}(X_\Zar)\morphism\cat{PAb}(X_\et)$ via
	\begin{equation*}
		\Global(U,\zeta_X^\sharp\Gg)=\Global(\text{image of $U\rightarrow X$},\Gg)\,.
	\end{equation*}
	It's easy to see that $\zeta_X^\sharp$ is left-adjoint to the forgetful functor $\cat{PAb}(X_\et)\morphism\cat{PAb}(X_\Zar)$. Now we can define $\zeta_X^*=(\zeta_X^\sharp)^\Sh$ to obtain a left-adjoint of $\zeta_{X,*}$. This functor is exact, as can be seen by examining the stalks of $\zeta_{X,*}\Gg$ at geometric points.
\end{proof*}
