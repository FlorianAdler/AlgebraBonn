...
\begin{proof}[Proof of \cref{prop:Quillen+}]
	...
	\begin{alphanumerate}
		\item[\emph{Step~1}.]\itshape We construct $X^+$ in the case where $\pi_1(X)$ is perfect itself.
	\end{alphanumerate}
	...
	\begin{alphanumerate}
		\item[\emph{Step~2.}] \itshape Still in the case where $\pi_1(X)$ is perfect, we use obstruction theory to verify that $\Hom_\An(X^+,Z)\isomorphism \Hom_\An(X,Z)$ is an equivalence for all hypoabelian anima $Z$.
	\end{alphanumerate}
	...
	\begin{alphanumerate}
		\item[\emph{Step~3.}] \itshape We construct $X^+$ in general and show that it satisfies the required universal property.
	\end{alphanumerate}
	...
	\begin{alphanumerate}
		\item[\emph{Step~4.}] \itshape We verify that $\IS[X]\isomorphism \IS[X^+]$ is an equivalence and that $(-)^+$ commutes with finite products.
	\end{alphanumerate}
...
\begin{equation*}
	k\cat{o}^*(X)=\pi_{-*}\Hom_\An(X,k\cat{o})=\pi_{-*}\hom_\Sp\big(\IS[X],B^\infty k\cat{o}\big)=(B^\infty k\cat{o})^{*}(X)
\end{equation*}
...
Consider the \emph{Hopf bundles} (see \cite[Examples~4.45--4.47]{Hatcher} for a construction)
\begin{equation*}
...
\begin{thm}[Bott periodicity]\label{thm:BottPeriodicity}
	...
	\end{equation*}
\end{thm}
We won't prove \cref{thm:BottPeriodicity}. If you want to read up on this, you'll find proofs in any book on topological $K$-theory but Fabian particularly recommends Bott's original proof in \cite{BottPeriodicity}.
...
\end{equation*}
(in \cref{prop:Ms-1}, we only saw how to localise elements from $\pi_0$, but the construction is straightforward to generalise to elements from arbitrary degrees). In this formluation we immediately see $K\cat{O},K\cat{U}\in\CAlg$. The associated cohomology theories were studied by Atiyah and Hirzebruch, and are known as \emph{\embrace{periodic} topological $K$-theory.}
...
\begin{alphanumerate}
	\item[\emph{Step~1.}] \itshape We'll massage the exterior power maps $\Lambda^i\colon \cat{Vect}_\IC(X)\morphism \cat{Vect}_\IC(X)$ on complex vector bundles long enough to turn them into maps $\lambda^i\colon k\cat{u}\morphism k\cat{u}$ of anima \embrace{not respecting any $\IE_\infty$-structures though}.
\end{alphanumerate}
...
\end{equation*}
to all of $k\cat{u}^0(X)$. However, knowing that $k\cat{u}^0(X)\simeq (\pi_0\cat{Vect}_\IC(X))^\grp$ doesn't help us as $\Lambda^i$ is not even a map of monoids! This can be fixed as follows: Instead of considering each $\Lambda^i$ on its own, we combine them into a single map
\begin{align*}
	\Lambda\colon \pi_0\cat{Vect}_\IC(X)&\morphism k\cat{u}^0(X)\llbracket t\rrbracket\\
	[V] & \longmapsto\sum_{i\geq 0}[\Lambda^iV]t^i\,,
\end{align*}
where $k\cat{u}^0(X)\llbracket t\rrbracket$ denotes the ring of power series over $k\cat{u}^0(X)$, which is a ring itself as denoted before \cref{thm:BottPeriodicity}. From the formula
\begin{equation*}
...
\end{equation*}
for all $x,y\in k\cat{u}^0(X)$. We've thereby reached our goal of Step~1.
\begin{alphanumerate}
	\item[\emph{Step~2.}] \itshape We show how one can extract ring homomorphisms $\psi^i\colon k\cat{u}^0(X)\morphism k\cat{u}^0(X)$ from the $\lambda^i$ by a purely algebraic procedure.
\end{alphanumerate}
...
Just as the $\Lambda^i$ in Step~1 behave nicer once combined into a single power series, the same happens for the $\lambda^i$. For better normalisation behaviour later one, we put
\begin{equation*}
...
\begin{proof}[Proof sketch of \cref{thm:QuillenKTheoryOfFiniteFields}]
	...
	\begin{alphanumerate}
		\item[\emph{Step~0.}] \itshape We recall some facts from the representation theory of finite groups.
	\end{alphanumerate}
	...
	\begin{alphanumerate}
		...
		\item For any field $k$, there are exterior power maps $\Lambda^i\colon \LMod_{k[G]}\morphism \LMod_{k[G]}$. Just as in Step~1 of \cref{par:AdamsOperations} (minus the technical mumbo-jumbo), these induce maps of sets $\lambda^i\colon R_k(G)\morphism R_k(G)$, turning the representation ring $R_k(G)=K_0(k[G])$ into a $\lambda$-ring. And just as in Step~2 of \cref{par:AdamsOperations}, we can then extract Adams operations $\psi^i\colon R_k(G)\morphism R_k(G)$ from them. In the case $k=\IC$, it follows from the constructions that $\operatorname{rep}\colon R_\IC(G)\morphism k\cat{u}^0(BG)$ from \itememph{a} commutes with the Adams operations on both sides.
	...
	\begin{alphanumerate}
		\item[\emph{Step~1.}] \itshape For any embedding $\rho\colon \ov{\IF}_p^\times\monomorphism \IC^\times$ we construct the famous \emph{Brauer lift}
		\begin{equation*}
	...
	\begin{alphanumerate}
		\item[\emph{Step~2.}] \itshape As a first step in that direction, we verify that the composition
		\begin{equation*}
	...
	\end{equation*}
	where the first sum ranges over all Frobenius orbits $[\mu]$. Using assertion \itememph{d} from Step~0 shows that applying $\psi^q$ only replaces $g$ by $g^q$. But the eigenvalues of $g^q$ are the $q\ordinalth$ powers of the eigenvalues of $g$ (for example by Jordan decomposition), and replacing each $\lambda$ by $\lambda^q$ only permutes the summands in $\sum_{\lambda\in[\mu]}\rho(\lambda)$, hence $\smash{\chi_{\ov{\IF}_p\otimes_{\IF_q}V}^\rho}$ is indeed invariant under the action of $\psi^q$.
	...
	\begin{alphanumerate}
		\item[\emph{Step~3.}]\itshape To get a compatible system of maps $B\!\GL_n(\IF_q)\morphism \fib(\psi^q-\id)$, we show that there are bijections
		\begin{gather*}
	...
	The second map is part of a \emph{Milnor exact sequence} in general (see for example the corresponding \href{https://ncatlab.org/nlab/show/lim^1+and+Milnor+sequences#MilnorSequences}{$n$Lab} article). This shows that it is surjective and its kernel is given by
	\begin{equation*}
...
\begin{proof}
	...
	\begin{equation*}
		\pi_0\Hom_\An\big(B\!\GL_n(\IF_q),\fib(\psi^q-\id)\big)\isomorphism \pi_0\Hom_\An\big(B\!\GL_n(\IF_q),k\cat{u}\big)^{\psi^q}
	\end{equation*}
	(Step~3 in the proof of \cref{thm:QuillenKTheoryOfFiniteFields}), we then obtain that $f_*$ agrees with the map induced by the right-hand square on fibres. Hence the claim follows from \cref{prop:AdamsOperations}\itememph{e} and the way we used it in the proof of \cref{thm:QuillenKTheoryOfFiniteFields}.
...
Fabian would like to end the chapter with some remarks on the Suslin rigidity theorem. But before we can do that, we need to introduce completions of spectra. This took us on a short detour in the lecture. I decided to make it a slightly longer detour, to elaborate more on the connection to derived completion in ordinary algebra and to explain one small technical step in the proof sketch of \cref{thm:QuillenKTheoryOfFiniteFields}.

Let's do the spectra case first. Given a spectrum $E\in\Sp$ and a prime $p$, we define the \emph{$p$-completion of $E$} as
\begin{equation*}
	\roof{E}_p\coloneqq \limit_{n\in\IN}E/p^nE\,,
\end{equation*}
where we define $E/p^nE$ as the pushout
\begin{equation*}
	\begin{tikzcd}
		E\rar["p^n"]\dar\drar[pushout] &E\dar\\
		0 \rar & E/p^nE
	\end{tikzcd}
\end{equation*}
Since $\hom_\Sp(-,E)$ preserves (but flips) fibre sequences, we see that $E/p^nE$ can equivalently be written as $\hom_\Sp((\IS/p^n\IS)[-1],E)$. This also shows
\begin{equation*}
	\roof{E}_p\simeq \hom_\Sp\big((\IS[p^{-1}]/\IS)[-1],E\big)\,,
\end{equation*}
which motivates the following lemma/definition:\refstepcounter{smallerdummy}
\numpar*{\thesmallerdummy. Lemma/Definition*}\label{lemdef*:pComplete}\itshape
For a spectrum $E$, the following are equivalent:
\begin{alphanumerate}
	\item $\hom_\Sp(\IS[p^{-1}],E)\simeq 0$.
	\item $\hom_\Sp(M,E)\simeq 0$ for all $p$-local spectra $M$.
	\item $E$ is equivalent to its $p$-completion $\roof{E}_p$.
\end{alphanumerate}
Such spectra are called \emph{$p$-complete}. The $p$-completion functor
\begin{equation*}
	(-)_p^\complete\colon \Sp\morphism \Sp_{p\mhyph\cat{comp}}
\end{equation*}
is a Bousfield localisation onto the full sub-$\infty$-category $\Sp_{p\mhyph\cat{comp}}\subseteq\Sp$ of $p$-complete spectra.\upshape
\begin{proof*}
	The implication \itememph{b} $\Rightarrow$ \itememph{a} is clear, and since $M\simeq M\otimes \IS[p^{-1}]$ by \cref{cor:ModRS-1}, the reverse implication follows from the the tensor-$\Hom$ adjunction (\cref{lem*:TensorHomAdjunction}).
	
	Using the the fact that $\IS[p^{-1}]\otimes \IS[p^{-1}]/\IS\simeq \IS[p^{-1}]/\IS[p^{-1}]\simeq 0$ together with the tensor-$\Hom$ adjunction, we see that the $p$-completion $\roof{E}_p\simeq \hom_\Sp((\IS[p^{-1}]/\IS)[-1],E)$ is indeed $p$-complete in the sense of \itememph{a}, which shows \itememph{c} $\Rightarrow$ \itememph{a}. Conversely, from the fibre sequence $(\IS[p^{-1}]/\IS)[-1]\morphism \IS\morphism\IS[p^{-1}]$ we get that $E$ is $p$-complete in the sense of \itememph{a} iff it equals its own $p$-completion. The final assertion about $(-)_p^\complete$ being a Bousfield localisation follows from Proposition~\labelcref{prop:LLaddendum}, as usual.
\end{proof*}
\refstepcounter{smallerdummy}
\numpar*{\thesmallerdummy. Derived Completion in General*}
...
\end{equation*}
In complete analogy with Lemma/Definition*~\labelcref{lemdef*:pComplete}, one shows that this is equivalent to $R\!\Hom_R(M,K)\simeq 0$ for all $M\in \Dd(R[s^{-1}])$. Moreover, if $C$ denotes the chain complex $(R\morphism R[s^{-1}])$ concentrated in degrees $0$ and $-1$, then the \emph{derived $s$-completion} functor
\begin{equation*}
	(-)_s^\complete\coloneqq R\!\Hom_R(C,-)\colon \Dd(R)\morphism \Dd(R)
\end{equation*}
is a Bousfield localisation onto the full sub-$\infty$-category $\Dd_{s\mhyph\mathrm{comp}}(R)\subseteq \Dd(R)$ of derived $s$-complete complexes. One might wonder perhaps why we use the complex $C$ rather than $(R[s^{-1}]/R)[-1]$, as was the case for spectra. This is because we have to take the derived quotient. In fact, one easily checks that $C[1]$ is the mapping cone of $R\morphism R[s^{-1}]$, hence it sits inside a pushout diagram
\begin{equation*}
	\begin{tikzcd}
	...
	\end{tikzcd}
\end{equation*}
Thus $C\simeq (R[s^{-1}]/^LR)[-1]$ and therefore our formula for derived completion looks exactly like in \cref{par:pCompletion}.

This can be generalised as follows: If $I=(s_1,\dotsc,s_n)\subseteq R$ is a finitely generated ideal, then a complex $K\in\Dd(R)$ is called \emph{derived $I$-complete} if it is derived $s$-complete for all $s\in I$. It turns out that this is equivalent to $K$ being derived $s_i$-complete for $i=1,\dotsc,n$. Again, there is a Bousfield localisation (\emph{derived $I$-completion})
\begin{equation*}
...
\end{proof*}
\refstepcounter{smallerdummy}
\numpar*{\thesmallerdummy. Derived Completion in the Noetherian Case*}
It is not true, in general, that derived $I$-completion is given by
\begin{equation*}
	\roof{K}_I\simeq R\!\limit_{n\in\IN}\left(K\otimes_R^LR/I^n\right)
\end{equation*}
(here $R\!\limit$ denotes the limit in the $\infty$-category $\Dd(R)$). And neither is it true (I think, but don't take my word for it) that derived completion is the left-derived functor of
\begin{equation*}
	\Lambda_I=\limit_{n\in\IN} -/I^n-\colon \Mod_R\morphism \Mod_R
\end{equation*}
in general. Fabian remarks that $\Lambda_I$ is neither left- nor right-exact in general\footnote{\dotso which confused me a bit, since $R^1\!\limit_{n\in\IN}M/I^nM=0$ holds by the Mittag-Leffler condition. However, this isn't enough to ensure right-exactness. For an inclusion $N\subseteq M$, the kernel of $N/I^nN\morphism M/I^nM$ is $(I^nM\cap N)/I^nN$, so we would need $R^1\!\limit_{n\in\IN}(I^nM\cap N)/I^nN=0$ instead---which isn't true in general.} \dotso but we need a right-exact functor to form left derivatives, right? Wrong! Grothendieck formulated his theory for arbitrary additive functors between abelian categories without any exactness requirements; it just won't be true anymore that $L_0\Lambda_I\simeq \Lambda_I$.

Both of the problems above go away if we assume $R$ is a noetherian ring. Indeed, in this case $\roof{K}_i\simeq R\!\limit_{n\in\IN}(K\otimes^LR/I^n)$ holds by \cite[\stackstag{0BKH}]{stacks-project}, which also implies that $(-)_I^\complete\simeq L\Lambda_I$, since it takes the correct values on free $R$-modules.
\refstepcounter{smallerdummy}
\numpar*{\thesmallerdummy. Derived Completion over $\IZ$ vs.\ Completion of Spectra}
Now let $p\in\IZ$ be a prime. Since $\IZ\morphism \IZ[p^{-1}]$ is injective, the complex $C=(\IZ\morphism \IZ[p^{-1}])$ concentrated in degrees $0$ and $-1$ is quasi-isomorphic to $(\IZ[p^{-1}]/\IZ)[-1]\simeq \mu_{p^\infty}[-1]$. Hence derived $p$-adic completion in $\Dd(\IZ)$ is given by
\begin{equation*}
	L\Lambda_p\simeq R\!\Hom_\IZ\big(\mu_{p^\infty}[-1],-\big)\colon \Dd(\IZ)\morphism \Dd_{p\mhyph\mathrm{comp}}(\IZ)\,.
\end{equation*}
...
\end{equation*}
To see where this sequence comes from, choose a projective resolution $0\morphism P_1\morphism P_0\morphism A\morphism 0$ (which we can do since $\IZ$ has global dimension $1$), note that $A[p^n]=\ker(P_1/p^nP_1\morphism P_0/p^nP_0)$ (since the right-hand side computes $\Tor_1^\IZ(\IZ/p^n,A)$) and play a bit around with exact sequences.

The upshot is that if $A$ has bounded $p$-torsion (for example, if $A$ is finitely generated), then there is an $n\gge 0$ such that $p^n\colon A[p^{2n}]\morphism A[p^n]$ is the zero morphism. Hence
\begin{equation*}
...
\begin{smalllem}\label{lem:HomotopyOfPCompletion}
	...
	\end{equation*}
	In particular, if $\pi_*(E)$ has degreewise bounded $p$-torsion, then its degreewise $p$-completion is $\pi_*(\roof{E}_p)$. Moreover, the homotopy groups of any $p$-complete spectrum are derived $p$-complete.
\end{smalllem}
\begin{proof*}
	...
	\begin{equation*}
		\begin{tikzcd}
			& & & 0\dar& \\
			0\rar & R^1\!\limit\limits_{n\in\IN}\pi_{i-1}(E)[p^n]\rar\dar[iso] & L_0\Lambda_p\big(\pi_i(E)\big) \rar\dar[dashed] & \Lambda_p\big(\pi_i(E)\big)\dar \rar & 0\\
			0\rar & R^1\!\limit\limits_{n\in\IN}\pi_i(E/p^nE)\rar & \pi_i(\roof{E}_p) \rar & \limit\limits_{n\in\IN}\pi_i(E/p^nE)\dar\rar & 0\\
...
\begin{smalllem}\label{lem:ArithmeticFractureSquare}
	\begin{alphanumerate}
		\item[\itememph{a^*}] The functors $-\otimes H\IQ\colon \Sp\morphism \Sp$ and $(-)_p^\complete\colon \Sp\morphism \Sp$ for all primes $p$ are jointly conservative.
		\item[\itememph{b^*}] The functors $-\otimes_\IZ \IQ\morphism \Dd(\IZ)\morphism \Dd(\IZ)$ and $-\otimes_\IZ^L\IZ/p\colon \Dd(\IZ)\morphism\Dd(\IZ)$ for all primes $p$ are jointly conservative. In particular, if $f\colon X\morphism Y$ is a map of anima such that
		\begin{equation*}
		...
		\begin{equation*}
			\begin{tikzcd}
				E\rar\dar\drar[pullback] & \displaystyle\prod_p\roof{E}_p\dar\\
...
\begin{proof*}[Proof of \cref{lem:ArithmeticFractureSquare}]
	For \itememph{a^*}, it suffices to show that $E\otimes H\IQ\simeq 0$ and $\roof{E}_p\simeq 0$ for all $p$ imply $E\simeq 0$. By the same argument as in \cref{lem*:DerivedNakayama}\itememph{a}, $\roof{E}_p\simeq 0$ implies that $E$ is $p$-local. If that's the case for all $p$, then $E\simeq E\otimes H\IQ$ by \cref{cor:DIQRationalSpectra}, hence the right-hand side vanishes iff $E$ vanishes.
	
	For part~\itememph{b^*}, we can use the same argument to show that $-\otimes_\IZ \IQ\colon \Dd(\IZ)\morphism \Dd(\IZ)$ and $(-)_p^\complete \Dd(\IZ)\morphism \Dd(\IZ)$ are jointly conservative. But $-\otimes_\IZ^L\IZ/p\colon \Dd_{p\mhyph\mathrm{comp}}(\IZ)\morphism \Dd(\IZ)$ is conservative by the derived Nakayama lemma (\cref{lem*:DerivedNakayama}\itememph{b}), hence the claim. The \enquote{in particular} follows as $H_*(-,\IQ)=H_*(C_\bullet(-)\otimes_\IZ\IQ)$ and $H_*(-,\IZ/p)=H_*(C_\bullet(-)\otimes_\IZ^L\IZ/p)$.
...
\end{proof*}
As a consequence of \cref{lem:ArithmeticFractureSquare}, to understand the \emph{algebraic $K$-theory spectrum} $K(F)=B^\infty k(F)$ of a field $F$, it suffices to understand its $p$-completions and its rationalisation. Surprisingly, it is the latter which causes problems in praxis, even though we have a formula for its homotopy groups: $K_i(F)\otimes\IQ=\operatorname{indec}_i H_*^{\grp}(\GL_\infty(F),\IQ)$ by \cref{cor:RationalKTheory}. But computing the right-hand side is an insanely hard problem and very much depends on $F$, as one can already see in the special case $K_1(F)=F^\times$ (by \cref{prop:WhiteheadsLemma}).

In contrast to that, the $\ell$-adic completions $K(F)_\ell^\complete$ are well understood and don't really depend on $F$:
\begin{thm}[Suslin's rigidity theorem]\label{thm:SuslinRigidity}
	...
	are equivalences.
\end{thm}
We won't prove \cref{thm:SuslinRigidity}. But note that $\Oo_p/\mm\simeq \ov{\IF}_p$. Hence we see that any pair of embeddings $\ov{\IQ}\monomorphism\ov{\IQ}_p$ and $\ov{\IQ}\monomorphism \ov{\IC}$ induces an equivalence $K(\ov{\IF}_p)_\ell^\complete\simeq (B^\infty k\cat{u})_\ell^\complete$. This fits with our calculations of their homotopy groups: Since $\pi_*(B^\infty k\cat{u})=\IZ[\beta]$ has degreewise bounded $\ell$-torsion, \cref{lem:HomotopyOfPCompletion} implies
\begin{equation*}
...
\begin{thm}[Friedlander, Fiedorowicz--Priddy {\cite{FiedorowiczPriddy}}]
	...
	\begin{align*}
		\gw^{\mathrm{-sym}}(\IF_q)_0&\isomorphism \fib(\psi^q-\id\colon B\cat{Spin}\morphism B\cat{Spin})\\
		\gw^{\mathrm{sym}}(\IF_q)_0&\isomorphism \fib(\psi^q-\id\colon B\cat{O}\morphism B\cat{O})\,,
	\end{align*}
	and in the even case we obtain
	\begin{gather*}
		\gw^{\mathrm{sym}}(\IF_{2^n})\simeq \gw^{\mathrm{even}}(\IF_{2^n})\isomorphism \fib(\psi^q-\id\colon B\cat{Spin}\morphism B\cat{Spin})\\
