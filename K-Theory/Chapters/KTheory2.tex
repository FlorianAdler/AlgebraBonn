...
\begin{proof}[Proof of \cref{prop:CompletionOfMonoidsFullyFaithful}]
	...
	\begin{alphanumerate}
		\item[\emph{Step~1.}] \itshape There exists a reasonable candidate $\decomp\colon {*/\cat{CSAn}}\morphism \cat{Mon}(\An)$ of an inverse functor \embrace{\enquote{decompletion}}.
	\end{alphanumerate}
...
\begin{proof*}
	...
	\end{equation*}
	By \cref{exm:MyFirstSpectra}\itememph{c}, the homotopy groups of the right-hand side are the homology groups of $R\!\Hom_\IZ(C_\bullet(E),A)$, which are the singular cohomology groups $H^*(E,A)$ (in negative degrees) by definition. Since $H^*(\IS[X],A)\simeq H^*(X,A)$ for all $X\in\An$ by \cref{lem:HomologyOfS}, plugging in the special case $E\simeq \IS[X]$ also shows $HA^*(X)\simeq H^*(X,A)$.
