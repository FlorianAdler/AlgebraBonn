...
\begin{prop}\label{prop:FreeMonoids}
	...
	\end{equation*}
\end{prop}
We postpone the proof of \cref{prop:FreeMonoids} indefinitely (but Fabian's notes give a sketch, see \cite[Chapter~II pp.~128--132]{KTheory}). By the explicit construction given there, the diagram
\begin{equation*}
	\begin{tikzcd}[column sep=large]
...
\begin{equation*}
	...
	\end{cases*}
\end{equation*}
(indeed, its well-known that $\Omega$ shifts homotopy groups down, so $B$ has to shift them up since $\Omega B\simeq \id$ by \cref{cor:CommutativeHorizontalAdjoints}). Hence the homotopy groups of both $B(KC)$ and $K(C[1])$ are those of $KC$, shifted up by one.
...
The forgetful functor $\CMon(\An)\morphism\An$ as a left adjoint $\Free^\CMon\colon \An\morphism \CMon(\An)$. Like \cref{prop:FreeMonoids}, we won't prove this, but Fabian gives a sketch in \cite[Chapter~II pp.\:128--132]{KTheory}, after our venture into the land of $\infty$-operads. On objects $X\in\An$, the left adjoint is given by
\begin{equation*}
...
\end{equation*}
Note however that neither $H_*(HC,A)$ nor $H_*(B^\infty X,A)$ are easy to compute! We will eventually (see \cref{cor*:SpectraCohomologyTheory}) find out that $H_*(X,A)=\Tor_*^\IS(\IS[X],HA)$ is given by \enquote{$\Tor$ over the sphere spectrum}.
...
\numpar*{\thesmallerdummy. Corollary}\itshape
...
We already know that $\Sp$ is complete and $\Omega^{\infty-i}\colon \Sp\morphism\An$ preserves limits for all $i$, see the summary before \cref{cor:RecognitionForInfiniteLoopSpaces}. We also know from \cref{propdef:StableInftyCategory} that $\Sp$ has finite colimits because it is stable. But in fact, $\Sp$ is even cocomplete! To see this, we will write $\Sp$ as a Bousfield localisation of a suitable cocomplete $\infty$-category of \emph{prespectra}, which will provide the desired colimits in $\Sp$ via \cref{cor:CoLimitsInBousfield}.
...
\begin{smalllem}\label{lem:piPreservesSequentialColimits}
	...
	\begin{alphanumerate}
		...
		\item[\itememph{b^*}] The functors $\pi_i\colon \Sp\morphism\cat{Ab}$ for $i\in\IZ$ and $\pi_i\colon {*/\An}\morphism \cat{Ab}$ for $i\geq 0$ commute with sequential colimits.
	\end{alphanumerate}
\end{smalllem}
\begin{proof*}
	First of, $\IN$ is weakly contractible, hence the colimits can be formed in $\An$ instead (\cref{rem*:CoLimitsIn(*/An)OrAn}). We can replace any sequential diagram $(X_1\morphism X_2\morphism \dotso)$ in $\An$ by a diagram $(X_1'\monomorphism X_2'\monomorphism \dotso)$ $\Kan$ such that all transition maps are cofibrations; then \cref{thm:HomotopyLimits} allows us to compute the colimit in $\Kan$ instead. Now write $\Omega\simeq \F_*((\IS^1,*),-)$ as the pointed mapping space out of $\IS^1$ (or any simplicial model for it, it doesn't even need to be Kan) and observe that the right-hand side commutes with filtered colimits in $\Kan$ because $\IS^1$ is compact (in fact, we can choose a finite simplicial model for it). This shows the first assertion.
...
Similarly to our construction of the embedding $\Sp(\Cc)\subseteq \cat{PSp}(\Cc)$, one can show that $\ev_{(0,0)}\colon \cat{PSp}(\Cc)\morphism */\Cc$ has a left adjoint $\snake{\Sigma}^\infty\colon {*/\Cc}\morphism \cat{PSp}(\Cc)$ if $\Cc$ has finite colimits. On objects $c\in */\Cc$, we define $\snake{\Sigma}^\infty c$ as follows: Recall $\cat{PSp}(\Cc)\subseteq\limit_{n\in\IN^\op}\Fun(\IN^2_{\leq(n,n)},*/\Cc)$, so we may imagine $\snake{\Sigma}^\infty c$ as a sequence $((\snake{\Sigma}^\infty c)_0,(\snake{\Sigma}^\infty c)_1,(\snake{\Sigma}^\infty c)_2,\dotsc)$. The $n\ordinalth$ piece $(\snake{\Sigma}^\infty c)_n$ is defined to be the left Kan extension of the of the following picture:
\begin{center}
	\begin{tikzpicture}[x=3em,y=3em,line cap=round]
		\node (00) at (0,0) {$c$};
		\node (10) at (1,0) {$*$};
...
\begin{smalllem}\label{lem:HomologyOfS}
	...
	\begin{equation*}
		...
		\end{cases*}\,.
	\end{equation*}
	More generally, we have $H_*(\IS[X],A)\simeq H_*(X,A)$ for all $X\in\An$.
\end{smalllem}
\begin{proof*}
	...
	\begin{align*}
		H_*(\IS,A)\simeq H_*(C_\bullet\IS\otimes_\IZ^LA)&\simeq \colimit_{i\in\IN}\colimit_{n\in\IN_{\geq i}}\snake{H}_{*+i}(\Omega^{n-i}\IS^n,A)\\
		&\simeq \colimit_{n\in\IN}\colimit_{i\in\IN_{\leq n}}\snake{H}_{*+i}(\Omega^{n-i}\IS^n,A)\\
		&\simeq \colimit_{n\in\IN}\snake{H}_{*+n}(\IS^n,A)\,,
	\end{align*}
	where we used that $i=n$ is terminal in $\IN_{\leq n}$ to get the third isomorphism. The bottom term clearly vanishes for $*\neq 0$ and equals $A$ for $*=0$, as we wished to show. The additional assertion follows from the fact that the functors $C_\bullet(-)\otimes_\IZ^LA\colon \An\morphism \Dd(\IZ)$ and $C_\bullet(\IS[-])\otimes_\IZ^LA\colon \An\morphism \Dd(\IZ)$ preserve colimits and agree on $*\in\An$ (as we just checked), hence they must be equal by \cref{thm:ColimitPreservingRepresentable}.
\end{proof*}
...
\begin{proof}
	...
	For simplicity, let me only prove that $\IOne$ is initial the special case $\Oo\simeq \IComm$, since the general case works just the same, but would require some new notation. So consider another algebra $A\in\Alg_\IComm(\Cc^\otimes)$, corresponding to a functor $A\colon \IGamma^\op\morphism \Cc^\otimes$ that preserves inerts and satisfies $p\circ A\simeq \id_{\IGamma^\op}$. Let's also write $A_n=A(\langle n\rangle)$. Since $A$ preserves inerts, we have $A_n\simeq(A_1,\dotsc,A_1)$ via the Segal maps. Also $\IOne_n\simeq (1_\Cc,\dotsc,1_\Cc)$ by construction. Now compute
	\begin{equation*}
	...
	and that taking $\Hom$ anima commutes with limits of the $\infty$-categories they are taken in. Now plug \cref{cor:HomInFunctorCats} into the right-hand side and use the fact that limits commute to obtain
	\begin{align*}
...
\end{prop}
\begin{proof*}
	We only prove the assertions about $\Sp$, since the arguments can be copied verbatim for the other cases. By \cref{prop:DayConvolution}\itememph{c}, $\operatorname{Day}((\IDigamma^\op)^\wedge,\Cc^\otimes)$ is symmetric monoidal and has all small operadic colimits. Since $\Sp(\Cc)\subseteq\Fun(\IDigamma^\op,\Cc)$ has a left adjoint, we may apply Lemma~\labelcref{lem:LaxMonoidalAdjoints}\itememph{b} to see that $\Sp^{\Op_\infty}(\Cc^\otimes)$ is again symmetric monoidal and its inclusion into $\operatorname{Day}((\IDigamma^\op)^\wedge,\Cc^\otimes)$ has a left adjoint $\operatorname{Day}((\IDigamma^\op)^\wedge,\Cc^\otimes)\morphism\Sp^{\Op_\infty}(\Cc^\otimes)$ which is strongly monoidal. Together with our computation of colimits in Bousfield localisations (\cref{cor:CoLimitsInBousfield}) this shows that $\Sp^{\Op_\infty}(\Cc^\otimes)$ has all small operadic colimits.
...
\numpar{}\label{par:EinftyRingSpectra}
...
In our picture where $\Sp$ corresponds to the $1$-category $\Ab$ of abelian groups, $\cat{CAlg}$ takes the place of the $1$-category $\cat{CRing}$ of commutative rings (which makes a lot of sense, as $\cat{CRing}\simeq\CAlg(\Ab^\otimes)$ by unwinding of definitions).

As a first example, recall from \cref{par:SpTensorProduct} that $\IS[-]\colon \An^\times\morphism\Sp^\otimes$ is strongly monoidal, and that $\CAlg(\An^\times)\simeq \CMon(\An)$ by \cref{thm:OMon}. Hence $\IS[-]$ upgrades to a functor
\begin{equation*}
...
As a second example, we would like to \enquote{import} ordinary rings into the theory of $\IE_\infty$-ring spectra. As it turns out, all commutative differential graded algebras (CGDAs) will be included as well. To prove this, we use the following:
...
\begin{proof*}[Proof sketch of \cref{cor:CRingToCAlgFullyFaithful}]
	...
	Fabian outlines two ways to do so. One can use the result from \cref{exm:MyFirstOperads}\itememph{f} to see that lax monoidal refinements of right adjoints are equivalent to oplax monoidal refinements of left adjoints, so it suffices that both candidates induce the same refinement of $\pi_0$.
...
\begin{cor*}\label{cor*:SpectraCohomologyTheory}
	...
	\begin{equation*}
		...
		\end{aligned}\,.
	\end{equation*}
	They satisfy the Eilenberg--Steenrod axioms. Moreover, if $A$ is an abelian group, then $HA_*(-)$ and $HA^*(-)$ coincide with the usual homology $H_*(-,A)$ and cohomology $H^*(-,A)$ with coefficients in $A$.
\end{cor*}
\begin{proof*}
	...
	Finally, we do our reality check for Eilenberg--MacLane spectra. The cohomology case is easy: The $(\IS[-],\Omega^\infty)$-adjunction gives
	\begin{align*}
		HA^i(X)\simeq \pi_{-i}\hom_\Sp\big(\IS[X],HA\big)&\simeq \pi_0\Omega^{\infty-i}\hom_\Sp\big(\IS[X],HA\big)\\
		&\simeq \pi_0\Hom_\An(X,\Omega^{\infty-i}HA)\,,
	\end{align*}
	and $\Omega^{\infty-i}HA\simeq K(A,i)$ by \cref{exm:MyFirstSpectra}\itememph{c}, hence the right-hand side is $H^i(X,A)$ by the Eilenberg--MacLane theorem (see Very Long Example~\cref{exm:EilenberMacLane}).
	
	The homology case is a bit more involved. First, \cref{lem:HomologyOfS} plus an easy argument implies $C_\bullet\IS[X]\simeq C_\bullet(X)$. Hence the unit of the $(C_\bullet,H)$-adjunction gives a canonical map $\IS[X]\morphism HC_\bullet(X)$ for any $X\in\An$. Combining this with the fact that $H$ is lax symmetric monoidal by \cref{lem:HLaxMonoidal}, we obtain maps
	\begin{equation*}
		\IS[X]\otimes HA\morphism HC_\bullet(X)\otimes HA\morphism H\big(C_\bullet(X)\otimes_\IZ^LA\big)\,.
	\end{equation*}
	Taking $\pi_*$ of the left-hand side gives $H_A*(X)$, and taking $\pi_*$ of the right-hand side gives $H_*(X,A)$ by \cref{exm:MyFirstSpectra}\itememph{c}. It's a well-known fact that a natural transformation between homology theory is an isomorphism iff it is one on $*\in\An$. But for $X=*$, both sides are $A$ in degree $0$ and $0$ else.
\end{proof*}
...
\begin{proof}[Proof sketch]
	...
	\end{equation*}
	where $M\colon \Ii\morphism \Cc$ is another diagram (the \enquote{module part} of $F$).
	
	We have to check that the there are functors
	\begin{align*}
	...
	\end{equation*}
	and the right-hand side is also a colimit in $\Cc^\otimes$ by \cref{rem*:OperadicLimits}, since $\Ii^\triangleleft$ is weakly contractible. Similarly, taking the colimit over a constant $\Ii^\triangleleft$-shaped $A$-diagram in $\Fun_{\IGamma^\op}(\IDelta^\op,\Cc^\otimes)$ gives $A$ again. After some unravelling, we see (I think) that this gives a way of computing $\Ii$-shaped colimits in $P$ by factorwise $\Ii^\triangleleft$-shaped colimits. A similar trick works for limits. I'll leave it to you to make this argument precise and also to check that the conditions from \cref{thm:AlgLModDescription} are satisfied.
\end{proof}
...
\begin{proof*}[Proof sketch of \cref{thm:D(R)IsModOverHR}]
	...
	We remark for future use that this argument also proves that $H\colon \Dd(R)\morphism \Sp$ is conservative and commutes with filtered colimits. Hence the same is true for the functor $H\colon \Dd(R)\morphism \Mod_{HR}$ we're interested in, since colimits and equivalences in $\Mod_{HR}$ can be detected on underlying spectra by \cref{cor:LModHasCoLimits} and the Segal condition from \cref{thm:AlgLModDescription}, respectively.
...
\begin{con}
	...
	\end{equation*}
	where $f\times p$ denotes the composition $[n]\times\Oo\morphism \IGamma^\op\times\IGamma^\op\morphism[\times]\IGamma^\op$. Let's compute the fibres $\ov{\Alg_\Oo(\Cc^\otimes)}^\otimes_n\simeq \ov{\Alg_\Oo(\Cc^\otimes)}^\otimes\times_{\IGamma^\op}\{\langle n\rangle\}$. Using the method from the proof sketch of \cref{prop:CocartesianMonoidalStructure}, we find that the Rezk nerve of the fibre over $\langle n\rangle$ is a simplicial anima $Y$ given by $Y_k\simeq F(\const {\langle n\rangle}\colon [k]\morphism \IGamma^\op)$. Plugging in the definitions, we get
	\begin{align*}
