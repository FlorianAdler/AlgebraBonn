...
\begin{thmdef}[Rezk, Joyal--Tierney, Lurie]\label{thmdef:RezkNerve}
	...
	\end{equation*}
	is an equivalence for all $n$ \embrace{$I_n$ is an iterated pushout of copies of $\Delta^1$, hence the pullback on the right-hand side}. We call $X$ \emph{complete} if the following equivalent conditions hold:
	\begin{alphanumerate}
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
To understand the completeness conditions a bit better, Fabian drew this picture for \cref{thmdef:RezkNerve}\itememph{b}. What condition~\itememph{b} is trying to say is that whenever a $3$-simplex $\Delta^3$ in $X_3$ has degenerate $\Delta^{\{0,2\}}$- and $\Delta^{\{1,3\}}$-edges as indicated, then all its edges are degenerate and the $3$-simplex itself is equivalent to a degenerate one. To get a grasp on condition~\itememph{d}, note that for any $\infty$-category $\Cc$ we have $\core\Cc$ encoded in $\N^r(\Cc)$ in two different ways: First in a direct fashion, as $\core\Cc\simeq \Hom_{\Cat_\infty}([0],\Cc)\simeq \N_0^r(\Cc)$, and second as the full subanima of $\core\Ar(\Cc)\simeq \N_1^r(\Cc)$ spanned by the equivalences in $\Cc$ (essentially because $\Delta^1\morphism \Delta^0$ is a localization). So $\N^r(\Cc)^\times$ should be constant with value $\core \Cc$.
...
\end{proof}
\numpar{The Quillen $Q$-Construction}
...
\begin{proof}
	...
	Now let $J_n\subseteq \TwAr([n])^\op$ be the subposet spanned by all $0$-simplices $(i\leq j)$ with $j\leq i+1$. In pictures, $J_n$ (for $n=3$) looks as follows:
	\begin{center}
	...
	\begin{equation*}
		\begin{tikzcd}
			F(0\leq 3)\rar[iso]\dar["\sim"{sloped,swap}]\drar[phantom,"\scriptscriptstyle/\!/\!/"] & F(1\leq 3)\dar[iso]\\
