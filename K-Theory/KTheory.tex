...
\begin{rem*}\label{rem*:CoLimitsIn(*/An)OrAn}
	For the proof, we'll need a functor $\Sigma\colon */\An\morphism */\An$ instead. It is induced by its unpointed variant, of course, but we can also define it by taking the above pushout in $*/\An$. In fact, I believe it doesn't matter whether the pushout is taken in $*/\An$ or $\An$ since $*/\An\morphism \An$ commutes with arbitrary colimits. Indeed, if $\Ii$ is any diagram shape, then a quick calculation shows $\Fun(\Ii,*/\An)\simeq \const */\Fun(\Ii,\An)$, so since adjunctions extend to slice categories we see that $\colimit_\Ii\colon \Fun(\Ii,\An)\shortdoublelrmorphism \An\noloc \const$ extends to an adjunction $\colimit_\Ii\colon \Fun(\Ii,*/\An)\shortdoublelrmorphism */\An\noloc \const$, and this proves what we want.
	
	Similarly, it doesn't matter whether we take the defining pullback of $\Omega$ in $\An$ or in $*/\An$. This can be seen as above, or simply by the fact that $*/\An\morphism \An$ has a left adjoint, so we can apply \cref{obs:AdjointsPreserveLimits}. Note however that $*/\An\morphism \An$ doesn't have a right-adjoint (it doesn't preserve the initial object).
\end{rem*}
