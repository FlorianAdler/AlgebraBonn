...
\begin{document}
	...
	Under no circumstances you should call Fabian by his last name! Also there will be oral exams, probably in the first or second week after the end of the lectures.
...
We won't prove \cref{thm:AllHomconstructionsBornEqual}. Lurie shows that \cref{par:HomC}\itememph{a} and \itememph{b} are equivalent in \cite[Proposition~5.2.1.11]{HA}, and to show equivalence of \itememph{c} and \itememph{d} you probably shouldn't go for a direct proof, but show that either one is equivalent to \itememph{b}. Fabian believes that if one works with any of these constructions long enough, then it will become obvious that the others are equivalent to it.

\numpar{More on the twisted arrow category}
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
As a bit of foreshadowing, Fabian mentioned that the first picture is related to the Quillen $Q$-construction (which will come up later), whereas the second is related to the Waldhausen $S$-construction (which we probably won't discuss in the course).
\begin{exc}
...
\begin{exm}\label{exm:Delta0Pushout}
	...
	\begin{center}
		\begin{tikzpicture}[x=0.6cm,y=0.6cm,line cap=round, line join=round]
			...
			\draw[-to,shorten <=1.25ex,shorten >=1.25ex,FabiansPurple!90!white] (B) to (C);
...
\end{prop}
\begin{proof*}
	Fabian claims (I think) this follows from \cite[Proposition~4.3.2.12]{HTT}, but Lurie's proof is rather laborious, so I'll give my own proof*. As it turns out, the arguments are pretty much the same as for the proof of Theorems~\labelcref{thm:ColimitPreservingRepresentable,thm:KanExtension} in Fabians notes. I'll freely reference later results (without running into circular arguments, I hope).
...
\lecture[Kan extensions and a version of \cref{thm:2Yoneda} for $\infty$-categories. Lots of examples to show how algebraic topology becomes a triviality after enough $\infty$-category theory.]{2020-11-12}The first goal for today is to discuss two closely related theorems, which provide an $\infty$-analogue of \cref{thm:2Yoneda}.
...
\end{defi}
In the official lectures notes (starting from \cite[Observation~I.56]{KTheory}) Fabian outlines two proofs of Theorems~\labelcref{thm:ColimitPreservingRepresentable,thm:KanExtension}, one due to Cisinski and one due to Lurie. However, Fabian believes he has a much simpler proof. There's still something I need to ask Fabian about the proof of \cref{thm:ColimitPreservingRepresentable}, but I can already present you a worked-out version of his brief sketch for \cref{thm:KanExtension}.
...
%	From \cref{thm:KanExtension} we obtain that $Y^{\Cc,*}$ admits a left adjoint $\Lan_{Y^\Cc}\colon \Fun(\Cc,\Dd)\morphism\Fun(\Pp(\Cc),\Dd)$. Since colimits in 
%\end{proof*}
A first example is given by $f_!\colon \Pp(\Cc)\morphism\Pp(\Dd)$ from \cref{cor:f_!:PC->PD}, which sends a presheaf $F\colon \Cc^\op\morphism\An$ to its left Kan extension $f_!F\colon \Dd^\op\morphism\An$ along $\Cc^\op\morphism\Dd^\op$. Moreover, we'll see that an inverse in \cref{thm:ColimitPreservingRepresentable} is given by $\Lan_{Y^\Cc}\colon \Fun(\Cc,\Dd)\morphism \Fun(\Pp(\Cc),\Dd)$, the functor of left Kan extension along the Yoneda embedding $Y^\Cc$, whose image essential image will thus be $\Fun^L(\Pp(\Cc),\Dd)$.
...
\begin{proof}
	...
	\end{equation*}
	of functors $\Cc^\op\morphism\An$. But regarding both sides as functors $\Pp(\Cc)\morphism\An^\op$ instead, note that they preserve colimits and they agree on representables since both become $\Hom_\Dd(F-,d)$ after precomposition with $Y^\Cc\colon \Cc\morphism\Pp(\Cc)$ (for the left-hand side we need to use \cref{cor:FullyFaithfulKanExtension}, for the right-hand side use Yoneda's lemma in the form of \cref{thm:Yoneda}\itememph{a}). Hence these two functors must be equivalent by \cref{thm:ColimitPreservingRepresentable}. To complete the proof that $R(d)$ is a right-adjoint object of $d$, we need to check that the above equivalence is actually induced by a map $\Lan_{Y^\Cc}F(R(d))\morphism d$, but this we get for free by plugging $R(d)$ into the above equivalence and taking the of $\id_{R(d)}\in\Nat(R(d),R(d))$ in the left-hand side.
\end{proof}
\numpar{Very Long Example}
...
\end{equation*}
I believe Fabian's argument for this in the lecture contained a small mistake, but to not interrupt the flow we'll discuss that later in \cref{rem*:UpgradeToPointed}. Now let $S^i$ be your favourite model for the $i$-sphere in anima and compute
\begin{align*}
	\pi_iK(C)&\simeq \pi_0\Hom_{*/\An}\big((S^i,*),K(C)\big)\\
	&\simeq \pi_0\Hom_{\Dd_{\geq 0}(\IZ)}\big(C_\bullet(S^i,*),C\big)\\
...
\begin{rem*}\label{rem*:UpgradeToPointed}
	...
	\begin{equation*}
		C_\bullet(-,*)\colon */\An\doublelrmorphism \Dd_{\geq 0}(\IZ)\noloc K\,,
	\end{equation*}
	where $C_\bullet(-,*)$ computes \emph{reduced} homology. For example, if $S^i$ denotes the $i$-sphere, then $C_\bullet(S^i,*)\simeq \cofib(\IZ[0]\morphism \IZ[0]\oplus \IZ[i])\simeq \IZ[i]$, as one would expect.
\end{rem*}
\begin{exm}
	...
	\begin{alphanumerate}
		...
		\begin{equation*}
			\text{\enquote{asscat}}\colon \cat{s}\An\doublelrmorphism \Cat_\infty\noloc \N^r\,.
		\end{equation*}
		The right adjoint $\N^r$ is called the \emph{Rezk nerve} and is a very important construction! If $\Cc$ is an $\infty$-category, \cref{cor:ExplicitAdjoint} provides the explicit description 
		\begin{equation*}
			\N^r(\Cc)=\Big([n]\mapsto \Hom_{\Cat_\infty}\big([n],\Cc\big)\simeq\core\Fun\big([n],\Cc\big)\Big)\,.
		\end{equation*}
		Beware that if $\Cc$ is a $1$-category, then $\N^r(\Cc)$ is different from the ordinary nerve $\N(\Cc)=([n]\mapsto \Hom_{\Cat_1}([n],\Cc))$, since $\Cat_1\subseteq\Cat_1^{(2)}$ is not fully faithful. In fact, $\N(\Cc)$ is a discrete simplicial anima, but $\N^r(\Cc)$ is usually not.
		\begin{thm**}[Rezk, Lurie, Tierney--Joyal]
			The Rezk nerve $\N^r\colon \Cat_\infty\morphism\cat{s}\An$ is fully faithful with essential image given by the complete Segal spaces.
		\end{thm**}
	\end{alphanumerate}
\end{exm}
\subsection{Bousfield Localizations}
\lecture[Bousfield localizations, limits and colimits inside them. Applications to derived categories of rings.]{2020-11-17}
\begin{propdef}\label{propdef:BousfieldLocalization}
	...
	\begin{alphanumerate}
		...
		\item A morphism $f\colon x\morphism y$ in $\Cc$ is taken to an equivalence by $L$ iff it is a left $R$-local equivalence, i.e.\ if the induced map
		\begin{equation*}
	...
	\end{alphanumerate}
	One then says that $L$ is a left Bousfield localization. In particular, $\Cc[\{\textup{$R$-local equiv.}\}^{-1}]$ is again locally small. Of course, there's also a dual notion of right Bousfield localizations.
\end{propdef}
\begin{proof}
...
\begin{lem}\label{lem:HomsInLocalization}
	...
	\end{alphanumerate}
\end{lem}
\numpar*{Warning \smash{\Attention}} Don't get too excited and think \enquote{Yeah, now \cref{thm:KanExtension} allows me to compute $\Hom$ anima in localizations!}, since the pointwise formula is useless here.
...
\begin{proof}
	...
	For the limit part, first note that $R$-local objects are closed under limits. Indeed, if $x\simeq\limit_{j\in \Jj}x_j$ with each $x_j$ an $R$-local object, then $\Hom_\Cc(-,x)\simeq \limit_{j\in \Jj}\Hom_\Cc(-,x_j)$ by the dual of \cref{cor:HomPreservesColimits}, hence $\Hom_\Cc(-,x)$ inverts $R$-local equivalences because each $\Hom_\Cc(-,x_j)$ does. Moreover, $R$-local equivalences between $R$-local objects are actual equivalences by Yoneda's lemma applied to the full sub-$\infty$-category of $\Cc$ spanned by $R$-local objects. Now \cref{propdef:BousfieldLocalization} shows that the unit
	\begin{equation*}
...
\begin{exm}
	...
	\begin{alphanumerate}
		...
		The argument also shows that $\An\simeq \N^c(\cat{CW})\morphism \N^c(\cat{Top})[\{\text{weak homotopy equiv.}^{-1}\}]$ is an equivalence. In particular, we get for free that CW approximation is $\infty$-functorial.
...
\begin{propdef}\label{propdef:derivedLocalization}
	...
	\begin{alphanumerate}
		...
		\item The cofibre $\cofib(\phi)$, i.e.\ the pushout
		\begin{equation*}
			\begin{tikzcd}
			...
			\end{tikzcd}
		\end{equation*}
		in $\Dd(R)$ satisfies $S\otimes_R^L\cofib(\phi)\simeq 0$.
	\end{alphanumerate}
	If these conditions are satisfied, $\phi$ is called a derived localization.
\end{propdef}
...
\begin{proof}[Proof of \cref{propdef:derivedLocalization}]
	...
	\begin{equation*}
		\begin{tikzcd}
			...
			0\rar & S\otimes_R^L\cofib(\phi)
		\end{tikzcd}
	\end{equation*}
	is a pullback square. This immediately implies $S\otimes_R^LS\simeq S$ iff $S\otimes_R^L\cofib(\phi)\simeq 0$, as desired.
