...
\begin{proof}[Sketch of a proof of \cref{thm:NewtonSlope}]
	...
	\end{equation*}
	To show \itememph{a} for $y_1$, it suffices to check that the first sum is divisible by $\pi^{d+1}$. As $x_iz^i=w_iz^d$ and $\pi=[z]$ in $\Oo_{C_{y_1}}$, we have
	\begin{equation*}
		\sum_{i=0}^d\theta_{y_1}([x_iz^i])=\pi^d\sum_{i=0}^d\theta_{y_1}([w_i])\,.
	\end{equation*}
	By construction of $z$ and the $w_i$ we have $\sum_{i=0}^dw_i=0$. Hence $\sum_{i=0}^d[w_i]\in\pi\IA_\inf$, whence we conclude that \itememph{a} holds for $y_1$. Since we were cut short by a sudden evaluation of the lecture, we will finish the induction next time.
\end{proof}
